...
// ── Microlink scraper (no API key needed) + ScrapingAnt fallback ────
async function scrapeProduct() {
  ...
  try {
    // Try Microlink first — free, no key needed
    let titel = '', bild = '', preis = '';

    const mlRes = await fetch(`https://api.microlink.io/?url=${encodeURIComponent(url)}&meta=false`);
    if (mlRes.ok) {
    ...
    // If Microlink didn't get enough, try ScrapingAnt
...
function extractFromDoc(doc) {
  ...
  for (const script of doc.querySelectorAll('script[type="application/ld+json"]')) {
    try {
      ...
      if (p) {
        titel = p.name || '';
        bild  = p.image?.url || p.image?.[0]?.url || p.image || '';
  ...
  // OG fallback
  const og = (prop) => doc.querySelector(`meta[property="${prop}"]`)?.content || doc.querySelector(`meta[name="${prop}"]`)?.content || '';
  if (!titel) titel = og('og:title') || og('twitter:title') || doc.title || '';
  if (!bild)  bild  = og('og:image') || '';
  if (!preis) preis = og('product:price:amount') || '';
