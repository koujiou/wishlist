...
// Handle email confirmation redirect — Supabase packt Tokens in den URL-Hash
if (window.location.hash && window.location.hash.includes('access_token')) {
  window.supabaseClient.auth.getSession().then(({ data: { session } }) => {
    if (session) {
      // Entferne den hässlichen Hash aus der URL, ohne die Seite neu zu laden
...
async function runSearch(query) {
  if (!currentUser) {
    searchResults.innerHTML = '<p class="search__result-empty">Bitte melde dich an, um zu suchen.</p>';
    return;
  }
  searchResults.innerHTML = '<p class="search__result-empty">Suche…</p>';
  try {
    const { data, error } = await window.supabaseClient
      .from('items')
      .select('id, produkttitel, bild, preis, link, list_id')
      .ilike('produkttitel', `%${query}%`)
      .limit(20);

    if (error) throw error;

    if (!data.length) {
      searchResults.innerHTML = '<p class="search__result-empty">Keine Ergebnisse gefunden.</p>';
      return;
    }

    searchResults.innerHTML = data.map(item => {
      const preis = item.preis != null ? `<span class="search__result-preis">€${parseFloat(item.preis).toFixed(2)}</span>` : '';
      const img   = item.bild
        ? `<img src="${item.bild}" alt="${item.produkttitel}" class="search__result-img" onerror="this.style.display='none'">`
        : `<div class="search__result-img search__result-img--placeholder"><i class="ri-shopping-bag-line"></i></div>`;
      const link  = item.link
        ? `href="${item.link}" target="_blank" rel="noopener"`
        : '';
      return `
        <a class="search__result-card" ${link}>
          ${img}
          <div class="search__result-body">
            <span class="search__result-titel">${item.produkttitel}</span>
            ${preis}
          </div>
          ${item.link ? '<i class="ri-external-link-line search__result-ext"></i>' : ''}
        </a>
      `;
    }).join('');
  } catch (err) {
    console.error('Search error:', err);
    searchResults.innerHTML = '<p class="search__result-empty">Fehler bei der Suche.</p>';
  }
}
...
async function handleAuth() {
  ...
  try {
    if (currentMode === 'login') {
      ...
      // Sofort setzen — nicht auf das asynchrone onAuthStateChange-Ereignis warten
...
if (logoutBtn) {
  logoutBtn.addEventListener('click', async () => {
    ...
    // Lösche localStorage sofort, damit die UI sofort reagiert
    Object.keys(localStorage)
      .filter(k => k.startsWith('sb-'))
      .forEach(k => localStorage.removeItem(k));
    currentUser = null;
    await applyAuthState();
    // Auch dem Supabase-Server mitteilen (fire and forget — nicht awaiten)
...
// Listener zuerst anhängen — immer aktiv, auch wenn getSession() hängt
...
// getSession als Best-Effort-Schnellwiederherstellung — wenn es vor
// onAuthStateChange feuert, wenden wir den Zustand sofort an
...
// ── Email-Bestätigungs-Hash-Handler ──────────────────────
// Wenn der Benutzer auf den Bestätigungslink klickt, leitet Supabase zurück
// mit #access_token=... in der URL. getSession() erkennt dies automatisch.
if (window.location.hash.includes('access_token')) {
  window.supabaseClient.auth.getSession().then(({ data: { session }, error }) => {
    ...
    if (session) {
      // Entferne den hässlichen Token aus der URL-Leiste
      history.replaceState(null, '', window.location.pathname);
    }
  });
}

// ── Single function die die UI mit dem Auth-Zustand synchronisiert ───────────
...
async function loadLists() {
  try {
    ...
    renderLists(data);
    // After lists load, fetch all item images for the swiper
    await updateHomeSwiper();
  } catch (err) { console.error('loadLists crashed:', err); renderLists([]); }
}

// ── Home swiper: fill with product images ─────────────────
const PLACEHOLDERS = [
...
const MIN_SLIDES = 8; // always keep enough for a wide screen

async function updateHomeSwiper() {
  try {
    const { data, error } = await window.supabaseClient
      .from('items')
      .select('bild, produkttitel')
      .not('bild', 'is', null)
      .neq('bild', '');

    if (error || !data) return;

    // Shuffle product images randomly
    const productImgs = data
      .map(i => ({ src: i.bild, alt: i.produkttitel }))
      .sort(() => Math.random() - 0.5);

    // Build slide list: product images first, pad with placeholders
    const slides = [...productImgs];
    let pIdx = 0;
    while (slides.length < MIN_SLIDES) {
      slides.push({ src: PLACEHOLDERS[pIdx % PLACEHOLDERS.length], alt: 'image' });
      pIdx++;
    }

    // Rebuild swiper wrapper
    const wrapper = document.getElementById('home-swiper-wrapper');
    if (!wrapper) return;

    wrapper.innerHTML = slides.map(s => `
      <article class="home__article swiper-slide">
        <img src="${s.src}" alt="${s.alt}" class="home__img"
          onerror="this.src='assets/img/home-book-1.png'">
      </article>
    `).join('');

    // Destroy and reinit swiper so it picks up new slides
    if (swiperHome) {
    ...
    console.error('updateHomeSwiper crashed:', err);
  }
}
...
let activeList   = null;
let scrapedData  = {}; // hält das zuletzt gescrapte Ergebnis
...
// ── Gespeicherte Elemente laden & rendern ────────────────────────────
...
// ── Scraper: Microlink zuerst, ScrapingAnt Fallback ───────
async function scrapeProduct() {
  const url = itemsUrlInput?.value.trim();
  if (!url) { setItemsFeedback('Bitte füge einen Produkt-Link ein.', true); return; }
  ...
  try {
    // ── 1. Versuche Microlink (kostenlos, kein Schlüssel) ──────────────────
    try {
      ...
      if (mlRes.ok) {
        ...
        console.log('Microlink result:', ml);
        if (ml.status === 'success' && ml.data?.title) {
          titel = ml.data.title || '';
          bild  = ml.data.image?.url || ml.data.logo?.url || '';
        }
      }
    } catch (mlErr) {
    ...
    // ── 2. ScrapingAnt Fallback (Browser-Modus + Proxy) ───
    if (!titel && SCRAPINGANT_KEY) {
      console.log('Trying ScrapingAnt...');
      const antUrl = new URL('https://api.scrapingant.com/v2/general');
      antUrl.searchParams.set('url',        url);
      antUrl.searchParams.set('x-api-key',  SCRAPINGANT_KEY);
      antUrl.searchParams.set('browser',    'true');   // JS rendering
      antUrl.searchParams.set('proxy_type', 'datacenter');
      antUrl.searchParams.set('return_page_source', 'true');

      const antRes = await fetch(antUrl.toString(), {
        signal: AbortSignal.timeout(30000)
      });
      console.log('ScrapingAnt status:', antRes.status);
      if (!antRes.ok) throw new Error(`ScrapingAnt Fehler: ${antRes.status}`);

      const html = await antRes.text();
      const doc  = new DOMParser().parseFromString(html, 'text/html');
      ({ titel, bild, preis } = extractFromDoc(doc));
    }

    // Clean up
    titel = titel.replace(/\s*[\|–\-]\s*(Thalia|Amazon|Zalando|eBay|Otto|MediaMarkt|Orell Füssli).*$/i, '').trim();
    preis = preis.toString().replace(/[^\d.,]/g, '').replace(',', '.').trim();
...
function extractFromDoc(doc) {
  ...
  for (const script of doc.querySelectorAll('script[type="application/ld+json"]')) {
    try {
      ...
      const p = Array.isArray(obj) ? obj.find(o => o['@type'] === 'Product')
        : obj['@type'] === 'Product' ? obj
        : obj['@graph']?.find?.(o => o['@type'] === 'Product');
      if (p) {
        titel = p.name || '';
        bild  = p.image?.url || p.image?.[0]?.url || (typeof p.image === 'string' ? p.image : '') || '';
  ...
  if (!bild)  bild  = og('og:image') || '';
  if (!preis) preis = og('product:price:amount') || og('og:price:amount') || '';
  if (!preis) {
    const el = doc.querySelector('[itemprop="price"]');
    if (el) preis = el.getAttribute('content') || el.textContent.trim();
  }
  return { titel, bild, preis };
}

// ── speichere gescraptes Produkt ─────────────────────────────────
async function saveScrapedItem() {
  ...
  appendItemCard(data);
  // Refresh swiper with the new image
  updateHomeSwiper();

  // setze scraper zurück
  itemsUrlInput.value         = '';
  scrapePreview.style.display = 'none';
  scrapeImg.src               = '';
  scrapeTitel.value           = '';
