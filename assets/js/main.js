...
const itemsTitelInput = document.getElementById('item-titel-input');
const itemsPreisInput = document.getElementById('item-preis-input');
const itemsBildInput  = document.getElementById('item-bild-input');
const itemsLinkInput  = document.getElementById('item-link-input');
...
function closeItemsModal() {
  itemsOverlay.classList.remove('show-modal'); activeList = null;
  if (itemsListEl)    itemsListEl.innerHTML = '';
  if (itemsTitelInput) itemsTitelInput.value = '';
  if (itemsPreisInput) itemsPreisInput.value = '';
  if (itemsBildInput)  itemsBildInput.value  = '';
  if (itemsLinkInput)  itemsLinkInput.value  = '';
...
async function loadItems(listId) {
  ...
  try {
    const { data, error } = await window.supabaseClient.from('items').select('*').eq('list_id', listId).order('created_at', { ascending: true });
    if (error) { itemsListEl.innerHTML = '<p class="items-modal__empty">Fehler beim Laden.</p>'; return; }
    renderItems(data);
  } catch (err) { console.error('loadItems crashed:', err); itemsListEl.innerHTML = '<p class="items-modal__empty">Fehler beim Laden.</p>'; }
}
function renderItems(items) {
  if (!itemsListEl) return;
  itemsListEl.innerHTML = '';
  if (items.length === 0) { itemsListEl.innerHTML = '<p class="items-modal__empty">Noch keine Produkte. Füge dein erstes hinzu!</p>'; return; }
...
function appendItemCard(item) {
  ...
  const linkBtn = item.link
    ? `<a href="${item.link}" target="_blank" rel="noopener" class="item-card__link" aria-label="Produkt öffnen"><i class="ri-external-link-line"></i></a>`
    : '';
...
async function addItem() {
  ...
  const titel = itemsTitelInput?.value.trim();
  const preis = itemsPreisInput?.value !== '' ? parseFloat(itemsPreisInput.value) : null;
  const bild  = itemsBildInput?.value.trim() || null;
  const link  = itemsLinkInput?.value.trim() || null;
  if (!titel) { itemsFeedback.textContent = 'Bitte gib einen Produkttitel ein.'; itemsFeedback.style.color = '#e53e3e'; itemsTitelInput?.focus(); return; }
  itemsAddBtn.disabled = true; itemsFeedback.textContent = 'Wird gespeichert…'; itemsFeedback.style.color = 'hsl(228,20%,60%)';
  const { data, error } = await window.supabaseClient.from('items').insert({ produkttitel: titel, preis, bild, link, list_id: activeList.id, user_id: currentUser.id }).select().single();
  itemsAddBtn.disabled = false;
  if (error) { itemsFeedback.textContent = 'Fehler: ' + error.message; itemsFeedback.style.color = '#e53e3e'; return; }
  itemsFeedback.textContent = '✓ Produkt hinzugefügt!'; itemsFeedback.style.color = '#38a169';
  appendItemCard(data);
  if (itemsTitelInput) itemsTitelInput.value = ''; if (itemsPreisInput) itemsPreisInput.value = ''; if (itemsBildInput) itemsBildInput.value = ''; if (itemsLinkInput) itemsLinkInput.value = '';
  setTimeout(() => { if (itemsFeedback) itemsFeedback.textContent = ''; }, 2000);
}
async function deleteItem(id, cardEl) {
  cardEl.style.transition = 'opacity 0.2s, transform 0.2s'; cardEl.style.opacity = '0'; cardEl.style.transform = 'scale(0.95)';
  ...
  setTimeout(() => {
    cardEl.remove();
    if (itemsListEl && !itemsListEl.querySelector('.item-card')) itemsListEl.innerHTML = '<p class="items-modal__empty">Noch keine Produkte. Füge dein erstes hinzu!</p>';
  }, 200);
}

if (itemsClose)   itemsClose.addEventListener('click', closeItemsModal);
if (itemsAddBtn)  itemsAddBtn.addEventListener('click', addItem);
if (itemsOverlay) itemsOverlay.addEventListener('click', e => { if (e.target === itemsOverlay) closeItemsModal(); });
if (itemsTitelInput) itemsTitelInput.addEventListener('keydown', e => { if (e.key === 'Enter') addItem(); });
